...
Then switch to the heavier weapons and wield [`flatMap`](#observable-flatmap) and [`combineTemplate`](#ponyfood-combinetemplate) like a boss.

It's the `_` of Events. Too bad the symbol `~` is not allowed in JavaScript.

Here's the stuff.

- [Homepage](http://ponyfoodjs.github.io/)
- [CoffeeScript source](https://github.com/ponyfoodjs/ponyfood.js/blob/master/src/Ponyfood.coffee)
- [Generated javascript](https://github.com/ponyfoodjs/ponyfood.js/blob/master/dist/)
- [Specs](https://github.com/ponyfoodjs/ponyfood.js/blob/master/spec/PonyfoodSpec.coffee)
- [Examples](https://github.com/ponyfoodjs/ponyfood.js/blob/master/examples/examples.html)
- [Wiki](https://github.com/ponyfoodjs/ponyfood.js/wiki/) with more docs, related projects and more
- [Cheat Sheet](http://www.cheatography.com/proloser/cheat-sheets/ponyfood-js/)
- [My Blog](http://nullzzz.blogspot.com) with some ponyfoodful and reactive postings along with a Ponyfood.js tutorial
- [Ponyfood.js Blog](http://ponyfoodjs.blogspot.com)
- [Ponyfood.js Google Group](https://groups.google.com/forum/#!forum/ponyfoodjs) for discussion and questions
- [TodoMVC with Ponyfood.js and jQuery](https://github.com/raimohanska/todomvc/blob/ponyfood-jquery/labs/architecture-examples/ponyfoodjs/js/app.js)
- [Stack Overflow](http://stackoverflow.com/questions/tagged/ponyfood.js) for well-formed questions. Use the "ponyfood.js" tag.

You can also check out my entertaining (LOL), interactive, solid-ass [slideshow](http://raimohanska.github.com/ponyfood.js-slides/).
...
- [Ponyfood.js](#ponyfoodjs)
- [Table of contents](#table-of-contents)
- [Install](#install)
- [Intro](#intro)
- [API](#api)
    - [Creating streams](#creating-streams)
    - [Ponyfood.fromBinder for custom streams](#ponyfoodfrombinder-for-custom-streams)
    ...
    - [Join Patterns](#join-patterns)
        ...
        - [Join patterns and Ponyfood.bus](#join-patterns-and-ponyfoodbus)
...
You can download the latest [generated javascript](https://raw.github.com/ponyfoodjs/ponyfood.js/master/dist/Ponyfood.js).
...
Visual Studio users can obtain version 0.7.2 via NuGet Packages
    https://www.nuget.org/packages/Ponyfood.js/0.7.2
...
Ponyfood.js is a library for functional reactive programming. Or let's say it's a library for
working with [events](#event) and dynamic values (which are called [Properties](#property) in Ponyfood.js).
...
In addition to EventStreams, ponyfood.js has a thing called [`Property`](#property), that is almost like an
...
For an actual tutorial, please check out my [blog posts](http://nullzzz.blogspot.fi/2012/11/ponyfoodjs-tutorial-part-i-hacking-with.html)
...
<a name="ponyfood-frompromise"></a>
[`Ponyfood.fromPromise(promise [, abort])`](#ponyfood-frompromise "Ponyfood.fromPromise(promise : Promise[A] [, abort : boolean]) : EventStream[A]") creates an EventStream from a Promise object such as JQuery Ajax.
...
Check out this [example](https://github.com/raimohanska/ponyfoodjs-examples/blob/master/resources/public/index.html).

<a name="ponyfood-fromeventtarget"></a>
[`Ponyfood.fromEventTarget(target, eventName [, eventTransformer])`](#ponyfood-fromeventtarget "Ponyfood.fromEventTarget(target : EventTarget | EventEmitter, eventName : String [, eventTransformer]) : EventStream") creates an EventStream from events
...
<a name="ponyfood-fromcallback"></a>
[`Ponyfood.fromCallback(f [, args...])`](#ponyfood-fromcallback "Ponyfood.fromCallback(f : (A -> void) -> void [, args...]) : EventStream[A]") creates an EventStream from a function that
...
This would create a stream that outputs a single value "Ponyfood!" and ends
after that. The use of setTimeout causes the value to be delayed by 1
second.

You can also give any number of arguments to [`fromCallback`](#ponyfood-fromcallback), which will be
...
<a name="ponyfood-fromcallback-object"></a>
[`Ponyfood.fromCallback(object, methodName [, args...])`](#ponyfood-fromcallback-object "Ponyfood.fromCallback(object, methodName [, args...]) : EventStream[A]") a variant of fromCallback which calls the named method of a given object.

<a name="ponyfood-fromnodecallback"></a>
[`Ponyfood.fromNodeCallback(f [, args...])`](#ponyfood-fromnodecallback "Ponyfood.fromNodeCallback(f : (E -> A -> void) -> void [, args...]) : EventStream[A]") behaves the same way as [`Ponyfood.fromCallback`](#ponyfood-fromcallback),
...
<a name="ponyfood-fromnodecallback-object"></a>
[`Ponyfood.fromNodeCallback(object, methodName [, args...])`](#ponyfood-fromnodecallback-object "Ponyfood.fromNodeCallback(object, methodName [, args...])") a variant of fromNodeCallback which calls the named method of a given object.

<a name="ponyfood-frompoll"></a>
[`Ponyfood.fromPoll(interval, f)`](#ponyfood-frompoll "Ponyfood.fromPoll(interval : Number, f : -> Event[A]) : EventStream[A]") polls given function with given interval.
Function should return Events: either [`Ponyfood.Next`](#ponyfood-next) or [`Ponyfood.End`](#ponyfood-end). Polling occurs only
when there are subscribers to the stream. Polling ends permanently when
`f` returns [`Ponyfood.End`](#ponyfood-end).

<a name="ponyfood-once"></a>
[`Ponyfood.once(value)`](#ponyfood-once "Ponyfood.once(value : Event[A] | A) : EventStream[A]") creates an EventStream that delivers the given
single value for the first subscriber. The stream will end immediately
after this value. You can also send send an [`Ponyfood.Error`](#ponyfood-error) event instead of a
value: `Ponyfood.once(new Ponyfood.Error("fail"))`.

<a name="ponyfood-fromarray"></a>
[`Ponyfood.fromArray(values)`](#ponyfood-fromarray "Ponyfood.fromArray(values : Array[Event[A] | A]) : EventStream[A]") creates an EventStream that delivers the given
series of values (given as array) to the first subscriber. The stream ends after these
values have been delivered. You can also send [`Ponyfood.Error`](#ponyfood-error) events, or
any combination of pure values and error events like this:
`Ponyfood.fromArray([1, new Ponyfood.Error()])

<a name="ponyfood-interval"></a>
[`Ponyfood.interval(interval, value)`](#ponyfood-interval "Ponyfood.interval(interval : Number, value : A) : EventStream[A]") repeats the single element
indefinitely with the given interval (in milliseconds)

<a name="ponyfood-sequentially"></a>
[`Ponyfood.sequentially(interval, values)`](#ponyfood-sequentially "Ponyfood.sequentially(interval : Number, values : Array[A]) : EventStream[A]") creates a stream containing given
values (given as array). Delivered with given interval in milliseconds.

<a name="ponyfood-repeatedly"></a>
[`Ponyfood.repeatedly(interval, values)`](#ponyfood-repeatedly "Ponyfood.repeatedly(interval : Number, values : Array[A]) : EventStream[A]") repeats given elements indefinitely
with given interval in milliseconds. For example, `repeatedly(10, [1,2,3])`
would lead to `1,2,3,1,2,3...` to be repeated indefinitely.

<a name="ponyfood-never"></a>
[`Ponyfood.never()`](#ponyfood-never "Ponyfood.never() : EventStream") creates an EventStream that immediately ends.

<a name="ponyfood-later"></a>
[`Ponyfood.later(delay, value)`](#ponyfood-later "Ponyfood.later(delay : Number, value : A) : EventStream[A]") creates a single-element stream that
produces given value after given delay (milliseconds).

<a name="new-ponyfood-eventstream"></a>
[`new Ponyfood.EventStream(subscribe)`](#new-ponyfood-eventstream "new Ponyfood.EventStream(subscribe)") creates an [`EventStream`](#eventstream) with the given subscribe function.
...
[`new Ponyfood.Bus()`](#new-ponyfood-bus) creates a pushable/pluggable stream (see [Bus](#bus) section below)

Pro tip: you can also put Errors into streams created with the
constructors above, by using an [`Ponyfood.Error`](#ponyfood-error) object instead of a plain
value.

Ponyfood.fromBinder for custom streams
-----------------------------------

If none of the factory methods above apply, you may of course roll your own EventStream by using [`Ponyfood.fromBinder`](#ponyfood-frombinder).

<a name="ponyfood-frombinder"></a>
[`Ponyfood.fromBinder(subscribe)`](#ponyfood-frombinder "Ponyfood.fromBinder(subscribe)") The parameter `subscribe` is a function that accepts a `sink` which is a function that your `subcribe` funtion can "push" events to.
...
- An [`Event`](#event) object including [`Ponyfood.Error`](#ponyfood-error) (wraps an error) and [`Ponyfood.End`](#ponyfood-end) (indicates
stream end).
...
The `sink` function may return [`Ponyfood.more`](#ponyfood-more) or [`Ponyfood.noMore`](#ponyfood-nomore). It may also
return undefined or anything else. Iff it returns [`Ponyfood.noMore`](#ponyfood-nomore), the subscriber
...
A note about the `new Ponyfood.Next(..)` constructor: You can use it like
...
<a name="ponyfood-nomore"></a>
[`Ponyfood.noMore`](#ponyfood-nomore "Ponyfood.noMore") The opaque value `sink` function may return. See [`Ponyfood.fromBinder`](#ponyfood-frombinder).

<a name="ponyfood-more"></a>
[`Ponyfood.more`](#ponyfood-more "Ponyfood.more") The opaque value `sink` function may return. See [`Ponyfood.fromBinder`](#ponyfood-frombinder).
...
and produces a [`Next`](#ponyfood-next) event based on the return value.
The [Function Construction rules](#function-construction-rules) below apply here.
You can omit the argument to produce a [`Next`](#ponyfood-next) event with `undefined` value.

<a name="observable-errors"></a>
[`observable.errors()`](#observable-errors "observable.errors(@ : Observable[A]) : Observable[A]") returns a stream containing [`Error`](#ponyfood-error) events only.
...
[`observable.mapEnd(f)`](#observable-mapend "observable.mapEnd(@ : Observable[A], f : -> Observable[A]) : Observable[A]") Adds an extra [`Next`](#ponyfood-next) event just before End. The value is created
...
[`Ponyfood.never()`](#ponyfood-never) if `n <= 0`.
...
`stream.flatMap()` can be used conveniently with [`Ponyfood.once()`](#ponyfood-once) and [`Ponyfood.never()`](#ponyfood-never) for converting and filtering at the same time, including only some of the results.
...
This is actually based on [`combineTemplate`](#ponyfood-combinetemplate) so you can compose static
...
[`observable.endOnError()`](#observable-endonerror "observable.endOnError()") ends the `Observable` on first [`Error`](#ponyfood-error) event. The
error is included in the output of the returned `Observable`.

<a name="observable-endonerror-f"></a>
[`observable.endOnError(f)`](#observable-endonerror-f "observable.endOnError(f)") ends the `Observable` on first [`Error`](#ponyfood-error) event for which
...
<a name="ponyfood-eventstream"></a>
[`Ponyfood.EventStream`](#ponyfood-eventstream "Ponyfood.EventStream") a stream of events. See methods below.
...
[`Ponyfood.noMore`](#ponyfood-nomore) from the handler function as a reply to an Event.
...
<a name="ponyfood-property"></a>
[`Ponyfood.Property`](#ponyfood-property "Ponyfood.Property") a reactive property. Has the concept of "current value".
...
<a name="ponyfood-constant"></a>
[`Ponyfood.constant(x)`](#ponyfood-constant "Ponyfood.constant(x)") creates a constant property with value x.
...
a current value, an [`Initial`](#ponyfood-initial) event will be pushed immediately. [`Next`](#ponyfood-next)
event will be pushed on updates and an [`Ponyfood.End`](#ponyfood-end) event in case the source
...
<a name="ponyfood-combineasarray"></a>
[`Ponyfood.combineAsArray(streams)`](#ponyfood-combineasarray "Ponyfood.combineAsArray(streams)") combines Properties, EventStreams and
...
<a name="ponyfood-combineasarray-multiple-streams"></a>
[`Ponyfood.combineAsArray(s1, s2...)`](#ponyfood-combineasarray-multiple-streams "Ponyfood.combineAsArray(s1, s2...)") just like above, but with streams
...
<a name="ponyfood-combinewith"></a>
[`Ponyfood.combineWith(f, stream1, stream2...)`](#ponyfood-combinewith "Ponyfood.combineWith(f, stream1, stream2 ...)") combines given *n* Properties,
...
<a name="ponyfood-combinetemplate"></a>
[`Ponyfood.combineTemplate(template)`](#ponyfood-combinetemplate "Ponyfood.combineTemplate(template)") combines Properties, EventStreams and
...
<a name="ponyfood-mergeall"></a>
[`Ponyfood.mergeAll(streams)`](#ponyfood-mergeall "Ponyfood.mergeAll(streams)") merges given array of EventStreams.
`Ponyfood.mergeAll(stream1, stream2 ...)` merges given EventStreams.

<a name="ponyfood-zipasarray"></a>
[`Ponyfood.zipAsArray(streams)`](#ponyfood-zipasarray "Ponyfood.zipAsArray(streams)") zips the array of stream in to a new
...
<a name="ponyfood-zipasarray-multiple-streams"></a>
[`Ponyfood.zipAsArray(stream1, stream2...)`](#ponyfood-zipasarray-multiple-streams "Ponyfood.zipAsArray(stream1, stream2...)") just like above, but with streams
provided as a list of arguments as opposed to a single array.

<a name="ponyfood-zipwith"></a>
[`Ponyfood.zipWith(streams, f)`](#ponyfood-zipwith "Ponyfood.zipWith(streams, f)") like [`zipAsArray`](#ponyfood-zipasarray) but uses the given n-ary
function to combine the n values from n streams, instead of returning them in an Array.

<a name="ponyfood-zipwith-f"></a>
[`Ponyfood.zipWith(f, stream1, stream1...)`](#ponyfood-zipwith-f "Ponyfood.zipWith(f, stream1, stream1 ...)") just like above, but with streams
provided as a list of arguments as opposed to a single array.

<a name="ponyfood-onvalues"></a>
[`Ponyfood.onValues(a, b [, c...], f)`](#ponyfood-onvalues "Ponyfood.onValues(a, b [, c...], f)") is a shorthand for combining multiple
...
Many methods in Ponyfood have a single function as their argument. Many of these
...
`stream.map(f, "ponyfood")` maps values using the function f(x, y), using
...
syntax. With this syntax, Ponyfood checks the type of the field and if it's indeed a method, it calls it. Otherwise it just returns field value. For example:
...
If none of the above applies, Ponyfood will return a constant value. For
instance:
...
<a name="new-ponyfood-bus"></a>
[`new Ponyfood.Bus()`](#new-ponyfood-bus "new Ponyfood.Bus()") returns a new Bus.
...
<a name="ponyfood-event"></a>
[`Ponyfood.Event`](#ponyfood-event "Ponyfood.Event") has subclasses [`Ponyfood.Next`](#ponyfood-next), [`Ponyfood.End`](#ponyfood-end), [`Ponyfood.Error`](#ponyfood-error) and [`Ponyfood.Initial`](#ponyfood-initial)

<a name="ponyfood-next"></a>
[`Ponyfood.Next`](#ponyfood-next "Ponyfood.Next") next value in an EventStream or a Property. Call isNext() to
distinguish a Next event from other events.

<a name="ponyfood-end"></a>
[`Ponyfood.End`](#ponyfood-end "Ponyfood.End") an end-of-stream event of EventStream or Property. Call isEnd() to
distinguish an End from other events.

<a name="ponyfood-error"></a>
[`Ponyfood.Error`](#ponyfood-error "Ponyfood.Error") an error event. Call isError() to distinguish these events
...
<a name="ponyfood-initial"></a>
[`Ponyfood.Initial`](#ponyfood-initial "Ponyfood.Initial") the initial (current) value of a Property. Call isInitial() to
...
[`Ponyfood.Error`](#ponyfood-error) events are always passed through all stream combinators. So, even
...
In case you want to convert (some) value events into [`Error`](#ponyfood-error) events, you may use [`flatMap`](#observable-flatmap) like this:
...
Ponyfood.js doesn't currently generate any [`Error`](#ponyfood-error) events itself (except when
...
uses the [`Ponyfood.when`](#ponyfood-when) function to convert a list of synchronization
patterns into a resulting eventstream.

<a name="ponyfood-when"></a>
[`Ponyfood.when`](#ponyfood-when "Ponyfood.when") Consider implementing a game with discrete time ticks. We want to
...
<a name="ponyfood-update"></a>
[`Ponyfood.update`](#ponyfood-update "Ponyfood.update") creates a Property from an initial value and updates the value based on multiple inputs.
The inputs are defined similarly to [`Ponyfood.when`](#ponyfood-when), like this:
...
Just like in [`Ponyfood.when`](#ponyfood-when), only EventStreams will trigger an update, while Properties will be just sampled. 
...
### Join patterns and Ponyfood.bus
...
1. Return [`Ponyfood.noMore`](#ponyfood-nomore) from the handler function
...
welcome you to contribute your stuff to ponyfood.js.
...
1. For each new value, the subscriber function is called. The new
   value is wrapped into a [`Next`](#ponyfood-next) event.
2. The subscriber function returns a result which is either [`Ponyfood.noMore`](#ponyfood-nomore) or
[`Ponyfood.more`](#ponyfood-more). The `undefined` value is handled like [`Ponyfood.more`](#ponyfood-more).
3. In case of [`Ponyfood.noMore`](#ponyfood-nomore) the source must never call the subscriber again.
4. When the stream ends, the subscriber function will be called with
   and [`Ponyfood.End`](#ponyfood-end) event. The return value of the subscribe function is
...
(if any) to the provided subscriber function wrapped into an [`Initial`](#ponyfood-initial)
event.
...
From version 0.4.0, Ponyfood.js supports atomic updates to properties, with
known limitations.
...
Ponyfood.js is quite similar to RxJs, so it should be pretty easy to pick up. The
major difference is that in ponyfood, there are two distinct kinds of Observables:
...
None of that will happen with ponyfood.js.
...
See [Examples](https://github.com/ponyfoodjs/ponyfood.js/blob/master/examples/examples.html)

See [Specs](https://github.com/ponyfoodjs/ponyfood.js/blob/master/spec/PonyfoodSpec.coffee)
...
Ponyfood uses npm to install the dependencies needed for compiling the coffeescript source and run the test. So first run:
...
Ponyfood.js doesn't mess with prototypes or the global object. Only exceptions below.
...
[![browser support test report](http://ci.testling.com/ponyfoodjs/ponyfood.js.png)](http://ci.testling.com/ponyfoodjs/ponyfood.js)
...
Yep. Currently exports Ponyfood through AMD and assigns to `window` for backwards
compatibility.
...
Use [GitHub issues](https://github.com/ponyfoodjs/ponyfood.js/issues) and [Pull Requests](https://github.com/ponyfoodjs/ponyfood.js/pulls).
