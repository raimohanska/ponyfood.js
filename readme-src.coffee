...
doc.fn "Ponyfood.fromEventTarget(target : EventTarget | EventEmitter, eventName : String [, eventTransformer]) : EventStream", """
...
doc.fn "Ponyfood.fromCallback(f : (A -> void) -> void [, args...]) : EventStream[A]", """
...
doc.fnOverload "Ponyfood.fromCallback(object, methodName [, args...]) : EventStream[A]", "object", """
...
behaves the same way as [`Ponyfood.fromCallback`](#ponyfood-fromcallback),
...
doc.fnOverload "Ponyfood.fromNodeCallback(object, methodName [, args...])", "object", """
...
Function should return Events: either `Ponyfood.Next` or `Ponyfood.End`. Polling occurs only
when there are subscribers to the stream. Polling ends permanently when
`f` returns `Ponyfood.End`.
"""
...
after this value. You can also send send an `Ponyfood.Error` event instead of a
...
values have been delivered. You can also send `Ponyfood.Error` events, or
any combination of pure values and error events like this:
`Ponyfood.fromArray([1, new Ponyfood.Error()])
"""

doc.fn "Ponyfood.interval(interval : Number, value : A) : EventStream[A]", """
repeats the single element
indefinitely with the given interval (in milliseconds)
"""

doc.fn "Ponyfood.sequentially(interval : Number, values : Array[A]) : EventStream[A]", """
...
doc.fn "Ponyfood.never() : EventStream", """
creates an EventStream that immediately ends.
"""

doc.fn "Ponyfood.later(delay : Number, value : A) : EventStream[A]", """
...
- An `Event` object including `Ponyfood.Error` (wraps an error) and `Ponyfood.End` (indicates
stream end).
...
The `sink` function may return `Ponyfood.more` or `Ponyfood.noMore`. It may also
return undefined or anything else. Iff it returns `Ponyfood.noMore`, the subscriber
...
doc.fn "Ponyfood.noMore", """The opaque value `sink` function may return. See `Ponyfood.fromBinder`."""
doc.fn "Ponyfood.more", """The opaque value `sink` function may return. See `Ponyfood.fromBinder`."""
...
returns a stream containing [`Error`](#ponyfood-error) events only.
...
`stream.flatMap()` can be used conveniently with `Ponyfood.once()` and `Ponyfood.never()` for converting and filtering at the same time, including only some of the results.
...
ends the `Observable` on first [`Error`](#ponyfood-error) event. The
error is included in the output of the returned `Observable`.
"""

doc.fnOverload "observable.endOnError(f)", "f", """
ends the `Observable` on first [`Error`](#ponyfood-error) event for which
...
doc.fn "Ponyfood.EventStream", "a stream of events. See methods below."
...
`Ponyfood.noMore` from the handler function as a reply to an Event.
"""
...
event will be pushed on updates and an `Ponyfood.End` event in case the source
...
doc.fn "Ponyfood.combineAsArray(streams)", """
...
doc.fnOverload "Ponyfood.combineAsArray(s1, s2...)", "multiple-streams", """
...
doc.fn "Ponyfood.zipAsArray(streams)", """
...
doc.fnOverload "Ponyfood.zipAsArray(stream1, stream2...)", "multiple-streams", """
just like above, but with streams
provided as a list of arguments as opposed to a single array.
"""
...
doc.fnOverload "Ponyfood.zipWith(f, stream1, stream1 ...)", "f", """
just like above, but with streams
provided as a list of arguments as opposed to a single array.
"""

doc.fn "Ponyfood.onValues(a, b [, c...], f)", """
...
has subclasses `Ponyfood.Next`, `Ponyfood.End`, `Ponyfood.Error` and `Ponyfood.Initial`
"""

doc.fn "Ponyfood.Next", """
...
doc.text """
`Ponyfood.Error` events are always passed through all stream combinators. So, even
...
In case you want to convert (some) value events into [`Error`](#ponyfood-error) events, you may use `flatMap` like this:
...
uses the `Ponyfood.when` function to convert a list of synchronization
patterns into a resulting eventstream.
"""

doc.fn "Ponyfood.when", """
...
1. Return `Ponyfood.noMore` from the handler function
...
2. The subscriber function returns a result which is either `Ponyfood.noMore` or
`Ponyfood.more`. The `undefined` value is handled like `Ponyfood.more`.
3. In case of `Ponyfood.noMore` the source must never call the subscriber again.
4. When the stream ends, the subscriber function will be called with
   and `Ponyfood.End` event. The return value of the subscribe function is
