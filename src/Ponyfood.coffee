...
Ponyfood.fromBinder = (binder, eventTransformer = _.id) ->
  new EventStream describe(Ponyfood, "fromBinder", binder, eventTransformer), (sink) ->
    unbinder = binder (args...) ->
      ...
      for event in value
        ...
        if reply == Ponyfood.noMore or event.isEnd()
          # defer if binder calls handler in sync before returning unbinder
          if unbinder? then unbinder() else Ponyfood.scheduler.setTimeout (-> unbinder()), 0
          return reply
      reply

# eventTransformer - defaults to returning the first argument to handler
Ponyfood.$ = asEventStream: (eventName, selector, eventTransformer) ->
  [eventTransformer, selector] = [selector, null] if isFunction(selector)
  withDescription(this, "asEventStream", eventName, Ponyfood.fromBinder (handler) =>
    @on(eventName, selector, handler)
    => @off(eventName, selector, handler)
  , eventTransformer)

(jQuery ? (Zepto ? null))?.fn.asEventStream = Ponyfood.$.asEventStream
...
# Examples
#
#   Ponyfood.fromEventTarget(document.body, "click")
#   # => EventStream
#
#   Ponyfood.fromEventTarget (new EventEmitter(), "data")
#   # => EventStream
#
# Returns EventStream
Ponyfood.fromEventTarget = (target, eventName, eventTransformer) ->
  ...
  withDescription(Ponyfood, "fromEventTarget", target, eventName, Ponyfood.fromBinder (handler) ->
    sub.call(target, eventName, handler)
    -> unsub.call(target, eventName, handler)
  , eventTransformer)

Ponyfood.fromPromise = (promise, abort) ->
  withDescription(Ponyfood, "fromPromise", promise, Ponyfood.fromBinder (handler) ->
...
Ponyfood.later = (delay, value) ->
  withDescription(Ponyfood, "later", delay, value, Ponyfood.sequentially(delay, [value]))

Ponyfood.sequentially = (delay, values) ->
  index = 0
  withDescription(Ponyfood, "sequentially", delay, values, Ponyfood.fromPoll delay, ->
    value = values[index++]
...
Ponyfood.repeatedly = (delay, values) ->
  index = 0
  withDescription(Ponyfood, "repeatedly", delay, values, Ponyfood.fromPoll(delay, -> values[index++ % values.length]))

Ponyfood.spy = (spy) -> spys.push(spy)
...
liftCallback = (desc, wrapped) ->
  withMethodCallSupport (f, args...) ->
    stream = partiallyApplied(wrapped, [(values, callback) ->
      f(values..., callback)])
    withDescription Ponyfood, desc, f, args..., Ponyfood.combineAsArray(args).flatMap(stream)

Ponyfood.fromCallback = liftCallback "fromCallback", (f, args...) ->
  Ponyfood.fromBinder (handler) ->
    makeFunction(f, args)(handler)
    nop
  , ((value) -> [value, end()])

Ponyfood.fromNodeCallback = liftCallback "fromNodeCallback", (f, args...) ->
  Ponyfood.fromBinder (handler) ->
    makeFunction(f, args)(handler)
    nop
  , (error, value) ->
...
Ponyfood.fromPoll = (delay, poll) ->
  withDescription(Ponyfood, "fromPoll", delay, poll,
  (Ponyfood.fromBinder(((handler) ->
    id = Ponyfood.scheduler.setInterval(handler, delay)
    -> Ponyfood.scheduler.clearInterval(id)), poll)))

Ponyfood.interval = (delay, value) ->
  value = {} unless value?
  withDescription(Ponyfood, "interval", delay, value, Ponyfood.fromPoll(delay, -> next(value)))

Ponyfood.constant = (value) ->
  new Property describe(Ponyfood, "constant", value), (sink) ->
    sink (initial value)
    sink (end())
    nop

Ponyfood.never = -> withDescription(Ponyfood, "never", Ponyfood.fromArray([]))

Ponyfood.once = (value) -> withDescription(Ponyfood, "once", value, Ponyfood.fromArray([value]))

Ponyfood.fromArray = (values) ->
  assertArray values
  values = cloneArray(values)
  new EventStream describe(Ponyfood, "fromArray", values), (sink) ->
    unsubd = false
    send = ->
      if _.empty values
        sink(end())
...
Ponyfood.mergeAll = (streams...) ->
  if isArray streams[0]
    streams = streams[0]
  withDescription Ponyfood, "mergeAll", streams..., _.fold(streams, Ponyfood.never(), ((a, b) -> a.merge(b)))

Ponyfood.zipAsArray = (streams...) ->
  if isArray streams[0]
    streams = streams[0]
  withDescription Ponyfood, "zipAsArray", streams..., Ponyfood.zipWith(streams, (xs...) -> xs)

Ponyfood.zipWith = (f, streams...) ->
  if !isFunction(f)
    [streams, f] = [f, streams[0]]
  streams = _.map(((s) -> s.toEventStream()), streams)
  withDescription(Ponyfood, "zipWith", f, streams..., Ponyfood.when(streams, f))

Ponyfood.groupSimultaneous = (streams...) ->
  if (streams.length == 1 and isArray(streams[0]))
    streams = streams[0]
  sources = for s in streams
    new BufferingSource(s)
  withDescription(Ponyfood, "groupSimultaneous", streams..., Ponyfood.when(sources, ((xs...) -> xs)))

Ponyfood.combineAsArray = (streams...) ->
  if (streams.length == 1 and isArray(streams[0]))
    streams = streams[0]
  for stream, index in streams
    streams[index] = Ponyfood.constant(stream) if not (isObservable(stream))
  if streams.length
    sources = for s in streams
      new Source(s, true, false, s.subscribeInternal)
    withDescription(Ponyfood, "combineAsArray", streams..., Ponyfood.when(sources, ((xs...) -> xs)).toProperty())
  else
    Ponyfood.constant([])

Ponyfood.onValues = (streams..., f) -> Ponyfood.combineAsArray(streams).onValues(f)

Ponyfood.combineWith = (f, streams...) ->
  withDescription(Ponyfood, "combineWith", f, streams..., Ponyfood.combineAsArray(streams).map (values) -> f(values...))
...
Ponyfood.combineTemplate = (template) ->
  ...
  combinator = (values) ->
    ...
    for f in funcs
       f(ctxStack, values)
    rootContext
  withDescription(Ponyfood, "combineTemplate", template, Ponyfood.combineAsArray(streams).map(combinator))
...
class Observable
    ...
    convertArgsToFunction this, f, args, (f) ->
      withDescription this, "endOnError", @withHandler (event) ->
        if event.isError() && f(event.error)
          ...
          @push event
  take: (count) ->
    return Ponyfood.never() if count <= 0
      ...
      unsub = @subscribeInternal (event) =>
        if (event.hasValue())
          if (initSent && event.isInitial())
            Ponyfood.more # init already sent, skip this one
          else
          ...
          if event.isEnd()
            reply = sendInit()
          sink event unless reply == Ponyfood.noMore
  ...
  combine: (other, f) =>
    ...
    withDescription(this, "combine", other, f, 
      Ponyfood.combineAsArray(this, other)
        .map (values) ->
          combinator(values[0], values[1]))
  decode: (cases) -> withDescription(this, "decode", cases, @combine(Ponyfood.combineTemplate(cases), (key, values) -> values[key]))

  awaiting: (other) ->
    withDescription(this, "awaiting", other, 
      Ponyfood.groupSimultaneous(this, other)
  ...
  new EventStream describe(root, "flatMap" + (if firstOnly then "First" else ""), f), (sink) ->
    ...
    composite.add (__, unsubRoot) -> root.subscribeInternal (event) ->
      if event.isEnd()
        ...
        return Ponyfood.noMore if composite.unsubscribed
        ...
        composite.add (unsubAll, unsubMe) -> child.subscribeInternal (event) ->
          if event.isEnd()
            ...
            if event instanceof Initial
              ...
              event = event.toNext()
            reply = sink event
            unsubAll() if reply == Ponyfood.noMore
            reply
...
class EventStream extends Observable
  ...
  debounceImmediate: (delay) ->
    withDescription(this, "debounceImmediate", delay, @flatMapFirst (value) ->
      Ponyfood.once(value).concat(Ponyfood.later(delay).filter(false)))
  ...
  buffer: (delay, onInput = (->), onFlush = (->)) ->
    ...
    if not isFunction(delay)
      delayMs = delay
      delay = (f) -> Ponyfood.scheduler.setTimeout(f, delayMs)
    ...
    new EventStream describe(left, "merge", right), (sink) ->
      ...
      smartSink = (obs) -> (unsubBoth) -> obs.subscribeInternal (event) ->
        if event.isEnd()
          ...
          reply = sink event
          unsubBoth() if reply == Ponyfood.noMore
          reply
...
class Property extends Observable
  ...
  sample: (interval) =>
    withDescription(this, "sample", interval,
      @sampledBy Ponyfood.interval(interval, {}))
        ...
        if not pushing
          ...
          try
            ...
            for sub in tmp
              ...
              removeSub sub if reply == Ponyfood.noMore or event.isEnd()
            success = true
          finally
            pushing = false
  ...
  constructor: ->
    ...
    subscribeAll = (newSink) =>
      ...
      for subscription in cloneArray(subscriptions)
        subscribeInput(subscription)
      unsubAll
    super(describe(Ponyfood, "Bus"), subscribeAll)
    @plug = (input) =>
      return if ended
      sub = { input: input }
...
Ponyfood.when = (patterns...) ->
    return Ponyfood.never() if patterns.length == 0
    len = patterns.length
    ...
    resultStream = new EventStream describe(Ponyfood, "when", patterns...), (sink) ->
      triggers = []
      ends = false
      match = (p) ->
        for i in p.ixs
...
containsDuplicateDeps = (observables, state = []) ->
  checkObservable = (obs) ->
    if Ponyfood._.contains(state, obs)
      ...
      if deps.length
        state.push(obs)
        Ponyfood._.any(deps, checkObservable)
      else
        state.push(obs)
        false

  Ponyfood._.any observables, checkObservable

Ponyfood.update = (initial, patterns...) ->
  ...
  while (i > 0)
    ...
    i = i - 2
  withDescription(Ponyfood, "update", initial, patterns..., Ponyfood.when(patterns...).scan initial, ((x,f) -> f x))
...
if module?
  module.exports = Ponyfood # for Ponyfood = require 'ponyfoodjs'
  Ponyfood.Ponyfood = Ponyfood # for {Ponyfood} = require 'ponyfoodjs'
else
  if define? and define.amd?
    define [], -> Ponyfood
  @Ponyfood = Ponyfood # otherwise for execution context
